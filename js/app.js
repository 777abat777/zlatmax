(() => {
    ...
    function initSliders() {
        if (document.querySelector(".home-media__slider")) new core(".home-media__slider", {
            ...
            on: {
                init: function(swiper) {
                    const slides = document.querySelector(".fraction-home__total");
                    const total = document.querySelectorAll(".home-media__slide:not(.swiper-slide-duplicate)");
                    slides.innerHTML = total.length;
                },
                slideChange: function(swiper) {
                    const currentslide = document.querySelector(".fraction-home__current");
                    console.log(swiper);
    ...
    var onresize = function(e) {
        ...
        if (width > 991.98) document.querySelector(".catalog-header__back").classList.remove("back-visible");
    };
    window.addEventListener("resize", onresize);
    document.addEventListener("click", docEctions);
    function docEctions(e) {
        const targetElement = e.target;
        if (targetElement.closest("[data-parent]")) {
            const subMenuId = targetElement.dataset.parent;
            const subMenu = document.querySelector(`[data-submenu="${subMenuId}"]`);
            const activeitem = document.querySelector("._sub-menu-active");
            const activemenu = document.querySelector("._sub-menu-open");
            if (activemenu) if (activemenu.dataset.submenu != subMenuId) {
                activeitem.classList.remove("_sub-menu-active");
                activemenu.classList.remove("_sub-menu-open");
            }
        ...
        function menuOpen() {
            const sublist = document.querySelector(".menu-catalog__list");
            const back = document.querySelector(".catalog-header__back");
            const menuCatalog = document.querySelector(".menu-catalog__sub-menu");
            const activemenu = document.querySelector("._sub-menu-open");
            const openSublis = document.querySelector(".menu-catalog__list-open");
            e.preventDefault();
            if (targetElement.closest(".menu__catalog")) {
                back.classList.add("back-visible");
                sublist.classList.toggle("menu-catalog__list-open");
            }
            if (targetElement.closest(".icon-menu")) {
                sublist.classList.remove("menu-catalog__list-open");
                menuCatalog.style.display = "none";
                if (activemenu) activemenu.classList.remove("_sub-menu-open");
                back.classList.remove("back-visible");
            }
            if (targetElement.closest(".catalog-header__back")) {
                sublist.classList.remove("menu-catalog__list-open");
                if (!openSublis.classList.contains(".menu-catalog__list-open")) back.classList.remove("back-visible");
                if (activemenu) {
                    activemenu.classList.remove("_sub-menu-open");
                    sublist.classList.add("menu-catalog__list-open");
                    back.classList.add("back-visible");
                }
            }
        }
    ...
    function getAllSpoiler() {
        let column = document.querySelectorAll(".sub-menu-catalog__block");
        let category = document.querySelectorAll(".sub-menu-catalog__category");
        for (let i = 0; i < column.length; i++) {
            column[i].setAttribute("data-spollers", "991.98,max");
            column[i].setAttribute("data-one-spoller", "");
        }
        for (let i = 0; i < category.length; i++) category[i].setAttribute("data-spoller", "");
    }
    getAllSpoiler();
    window["FLS"] = true;
    isWebp();
